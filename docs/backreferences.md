...
**The same is true in `expand()`: a forward reference is rendered as the native `\N` there too, never substituted from the scan.** It's in fact *unconditionally* empty, not just where it textually precedes its group — even across an enclosing quantifier's iterations, since ECMAScript resets a capturing group to "not yet participated" at the start of every fresh attempt to match the atom it's nested in, so a backreference to that group never sees an earlier iteration's capture either:
