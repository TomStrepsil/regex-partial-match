...
**Captures of an incomplete match are the closest to what a full match reports.** Each group holds what a completion along the path the truncated match took captures, cut off at the end of the input, so a repeated group whose last iteration was cut short reports that partial iteration — `/(abc)+\1/` on `"abcab"` gives `m[1] === "ab"` — and `"abcabc"` then gives `"abc"`. More input can still replace that path with another: a group can grow, shrink, become `undefined` as a different alternative wins, or a different group can appear in its place. A match for which `hitEnd()` is `false` reports what the engine resolved, exactly as the original would.
