...
The atoms are still baked from the scan, so a capture the expanded regex resolves *differently* can still be truncated against the wrong text (`/^([ab])\1([ab])\2$/` accepts `"aaba"`). That is a separate imprecision in the scan, not in the expansion shape, and it needs the two to be reconciled rather than merely ordered.
