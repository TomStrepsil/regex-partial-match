...
**A forward reference is rendered the same way, and never from the scan, whatever the scan found.** `\N` written before group `N` opens, or referencing it while it's still open, is not guaranteed to be empty at that point — the same ECMA-262 clause governs it directly, matching empty if the group genuinely hasn't captured anything yet, or its current value if an enclosing quantifier's earlier iteration already gave it one. The scan, though, resolves it along a path where the stand-in has already run, and can report a value the reference could never have had at that point in that attempt. Substituting it demands text the original never required: `new PartialMatchRegExp(/^\1a(b)/)` accepted `"bab"`, which `/^\1a(b)/` rejects outright, since every match of it begins with `"a"`; `new PartialMatchRegExp(/^(\1a)$/)` (a self-reference, still open) accepted `"ba"` the same way, though `/^(\1a)$/` only ever matches `"a"`. Deferring to the native `\N` gets both cases right — empty where the group genuinely cannot have participated, and the group's real value where an enclosing quantifier's earlier iteration gave it one, as in `/^(?:\1(a))+$/` on `"aaa"`. The cost is the same one: that site is atomic, no mid-value partial matching.
...
A `\k<name>` referencing a name declared more than once is rendered this way unconditionally, for every occurrence of that name. ECMAScript permits a duplicate name only across disjoint alternatives — at most one occurrence can ever participate in a given match — but the walk records `closedGroupNames` in source order regardless of alternation structure, so it can't distinguish "closed in the alternative this reference can actually reach" from "closed in one it never could." 

The algorithm finds every name declared more than once once the walk finishes, and forces `forward` on every reference to it, unconditionally — including from the one alternative where that occurrence has genuinely already closed. This costs strictly more completeness than the plain forward case: `/^(?:(?<x>ab)\k<x>|z(?<x>q))$/` rejects the partial `"aba"` outright, even though it's a valid prefix of the full match `"abab"`, and even though the identical `\k<x>` in `/^(?<x>ab)\k<x>$/` (no duplicate) resolves it per-character as normal. Renaming the duplicate group is the only way around it — there's no way to make it precise without tracking which alternative each reference can actually reach, which the walk doesn't do.
