...
The parity tests added in `src/partialMatchRegExp/partialMatchRegExp.test.ts` ("parity with reference implementations") vary in how directly they reflect each source:
...
| `Matcher.hitEnd()`               | Semantic: did the engine read the end of the input?            | [`hitEnd(partial, match)`](../README.md#hitendpartial-partialmatchregexp-match-regexpexecarray-boolean) — same contract; conservative for a bounded greedy quantifier on a group fully taken at the end |
