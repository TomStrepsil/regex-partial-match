...
| `Matcher.hitEnd()`               | Semantic: did the engine read the end of the input?            | [`hitEnd(partial, match)`](../README.md#hitendpartial-partialmatchregexp-match-regexpexecarray-boolean) — same contract, approximated: conservative for a bounded greedy quantifier on a group fully taken at the end, and reports `false` after an actual end-read for the two [documented limits](../README.md#what-it-cannot-see) |
