...
- Split `compilePartial.ts` into a `compilePartial/` directory: `compilePartial()` itself stays in `index.ts`, `compiled.ts` holds the `Compiled`/`CompiledStatic`/`CompiledDynamic`/`DynamicPath` shapes, and every other helper (`asOptionalAtom`, `asNativeAtom`, `asPreScanPart`, `resolvedFromScan`, `caseFoldFlags`, `startsWithUnderFlags`, `longestBakedPrefixEndingInput`, `reclassifyLegacyEscapes`, `spliceOriginalSource`, `renderParts`, `toStatic`) gets its own file. `ANY_CAPTURED_TEXT` moved into `atomSyntax.ts` alongside the other rendering-syntax constants it's shared with
...
- `devEngines.runtime` now requires Node.js ≥24.12, which carries the V8 fix for [nodejs/node#60030](https://github.com/nodejs/node/issues/60030); the regression test for that case is skipped on an older runtime rather than asserting behaviour an unpatched engine can't provide

### Fixed

- A backreference no longer matches a *partial* copy of its captured text anywhere but at the end of input, so `new PartialMatchRegExp(/^(a?)\1(b)\2$/).exec("ab")` no longer reports a full match of `"ab"` — a string the pattern can never match, nor a prefix of one ([#89](https://github.com/TomStrepsil/regex-partial-match/issues/89))
- A backreference that runs the input out no longer truncates against a capture the match itself resolved differently, so `new PartialMatchRegExp(/^([ab])\1([ab])\2$/).exec("aaba")` no longer reports a full match of `"aaba"`; where the two disagree, the expansion is re-derived from the match's own captures
  - That same agreement check now compares under the pattern's own case-folding instead of case-sensitively, so `new PartialMatchRegExp(/^([ab])\1([ab])\2$/i).exec("aabA")` no longer reports a full match of `"aabA"` by having the disagreeing capture's baked value fail to match the input's case at all
  - The agreement check now tracks case-folding per backreference — a locally-scoped `(?i:...)`/`(?-i:...)` [modifier](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Regular_expressions/Modifier) around that backreference is honoured instead of only the pattern's own flags, while the pattern's `u`/`v` mode (which a modifier group can't itself scope) is still applied throughout. Where the pattern has `i` and a backreference is scoped `(?-i:...)`, [a V8 engine bug](https://github.com/nodejs/node/issues/60030) evaluating a disjunction inside a modifier group can still admit case-mismatched input on an unpatched engine — fixed by [nodejs/node#60732](https://github.com/nodejs/node/pull/60732) from Node.js 24.12.0 onward, but present on earlier 24.x builds and unpatched Chromium/Electron. See the [Backreferences](../README.md#backreferences) section of the README
  - `longestBakedPrefixEndingInput()` no longer builds one alternative per possible prefix length of the baked capture to find the longest one the input agrees with; that alternation's source was quadratic in the capture's length, and a several-thousand-character case-insensitive capture could exhaust memory compiling it. It now finds the same answer with the Knuth–Morris–Pratt failure function of `baked + sentinel + input`, linear in both time and source size
