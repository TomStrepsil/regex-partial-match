...
- A named backreference is no longer misclassified as forward, or its scanned value silently dropped, when its declaration and its `\k<name>` spell the group name differently (a literal character vs. `\uXXXX`/`\u{XXXX}`, e.g. `(?<a>ab)\k<a>`). Both the forward/backward classification and the capture-scan lookup now compare decoded names, matching the decoding `isComplete()`'s truncation probe already relied on for the same reason
