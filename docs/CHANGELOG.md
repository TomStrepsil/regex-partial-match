...
- A `\k<name>` in a pattern that declares no named group, and a bare `\k`, are walked as the [Annex B](https://tc39.es/ecma262/#sec-regular-expressions-patterns) literals they are rather than as backreferences — correcting prefix matching, `features` reporting, and `isComplete()` throwing on them
...
- The truncation probe decides whether a named reference inside a raw lookaround is genuine from the pattern's own declarations, rather than from a text scan of the source that can see a group name inside a character class
