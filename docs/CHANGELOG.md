...
- `isComplete(match: RegExpExecArray): boolean` on `PartialMatchRegExp`, reporting whether a match `exec()` returned is a match of the original pattern or merely a prefix of it — the distinction partial matching is named for, which the result previously discarded. Answered by re-running the compiled pattern sticky at `match.index` with an empty named group in front of each `|$(?![\s\S])` truncation branch: an empty group is zero-width and always succeeds, so the twin walks the identical path, and any marker returned defined is a branch the match actually took. Supported on both the static and the backreference paths
  ...
  - Documented that `isComplete()` itself always requires ES2018+ regardless of the pattern, since its probe uses named capturing groups internally, and corrected the cost note: the twin is cached once per instance only on the static path, while the backreference path caches it against one particular match
  - A [legacy escape](https://tc39.es/ecma262/#sec-regular-expressions-patterns) is now rewritten to a group-count-independent literal as it is reclassified, rather than being carried along as raw source text. Whether `\1` is a backreference or an octal escape, and whether `\k<name>` is a backreference or a literal `k<name>`, is decided against the pattern's capturing groups and names — which is exactly what the probe's markers add, so left as raw text `/^\1a/` gained a reference to a marker and `\k<name>` gained one to a name that does not exist

### Changed

- `compilePartial()` recognises a `\k<name>` in a pattern that declares no named group as the [Annex B](https://tc39.es/ecma262/#sec-regular-expressions-patterns) literal it is, rather than as a backreference, so such a pattern compiles on the static path instead of taking the per-input backreference path for a backreference it does not have. That path rebuilds a `RegExp` on every call, so `/^\k<none>tail/` now execs a partial input in 19ns rather than 825ns, and constructs around 1.2x faster. Both paths also read the same reclassified parts now, so no escape reaches either of them still spelled as a reference
- The expansion behind a backreference match is held on the match under a private symbol rather than in a per-instance `WeakMap`. The map was written to on every partial match whether or not `isComplete()` was ever called, and a `WeakMap.set` per match dominated that path — restoring it to within 6% of 1.2.0, from around 60% slower
- `isComplete()` no longer throws on an Annex B `\k<name>` identity escape inside a raw lookaround, such as `/^a(?!\k<bogus>)b/`. A raw lookaround is copied into the probe verbatim, so adding the probe's named markers reparsed that literal as a reference to a group that does not exist; named references inside a lookaround are now canonicalised the same way numeric ones already were
- The backreference path builds its per-input regex from an array of atoms rather than a concatenated source string, so the truncation markers can be placed in it, and records that expansion against the match it produced. Measured against the 1.2.0 worktree in a separate process, that leaves the path around 6% slower on the repeated-word benchmark
- `compilePartial()` joins its parts directly on the static path rather than routing them through `render()`, which has no backreferences to substitute there, and shares that return between both places a pattern turns out to have none
