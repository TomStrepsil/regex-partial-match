...
- A `\1`/`\k<name>` that can't yet have participated — written before its own group, or referencing it while still open, e.g. `\1` in `/^(\1a)$/` — is now left for the engine to resolve rather than taken from the capture scan, which could hand it a value from a path it never took
