...
- Updated npm in corepack packageManager to 11.19.0
...
- The truncation probe classifies each rendered part (backreference, raw lookaround, group open, truncation-ending atom) once via a shared `roleOf`, rather than two of its passes independently re-deriving the same classification from the rendering syntax.
